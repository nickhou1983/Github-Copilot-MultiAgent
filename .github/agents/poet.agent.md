...
如果判断需要配视频，使用 `azure-video-gen` Skill 提供的脚本生成视频：

```bash
python .github/skills/azure-video-gen/generate_video.py \
  --prompt "<视频描述，建议描述动态场景和运镜方式>" \
  --output "./output/<文件名>.mp4" \
  --size "1920x1080" \
  --n-seconds 5
```

> 注意：运行前需确保已安装依赖 `pip install -r .github/skills/azure-video-gen/requirements.txt` 并配置 `.env` 文件。视频生成是异步过程，可能需要等待数分钟。
